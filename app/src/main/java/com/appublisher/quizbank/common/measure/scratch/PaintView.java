...
public class PaintView extends View {
    ...
    //Background Color
    //背景色
    private int mBgColor = Color.parseColor("#33262B2D");
    ...
    private ArrayList<DrawShape> mDeleteDrawShapes = new ArrayList<>();
    ...
    private void setDefaultBitmap(Context context) {
        ...
        int height = dm.heightPixels - 2 * 45;
        //画布大小
        mBgBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
    }
    ...
    public boolean undo() {
        if (mDrawShapes != null && mDrawShapes.size() > 0) {
            mDeleteDrawShapes.add(mDrawShapes.get(mDrawShapes.size() - 1));
    ...
    public boolean redo() {
        if (mDeleteDrawShapes != null && mDeleteDrawShapes.size() > 0) {
            mDrawShapes.add(mDeleteDrawShapes.get(mDeleteDrawShapes.size() - 1));
            mDeleteDrawShapes.remove(mDeleteDrawShapes.size() - 1);
            invalidate();
        }
        ...
        return mDeleteDrawShapes != null && mDeleteDrawShapes.size() > 0;
    }

    public void clear() {
        mDrawShapes.clear();
        mDeleteDrawShapes.clear();
        invalidate();
        if (mOnDrawListener != null) {
            mOnDrawListener.afterDraw(mDrawShapes);
        }
